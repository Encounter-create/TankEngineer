...
export function renderSiege(ctx: CanvasRenderingContext2D, state: SiegeState, mx?: number, my?: number): void {
  ...
  for (let i = 0; i < (state.playerTanks?.length || 0); i++) {
    ...
    ctx.lineWidth = 2;
    const cy = t.pos.y + TANK_RADIUS + 6;
    ctx.beginPath(); ctx.arc(t.pos.x, cy, 6, 0, Math.PI * 2); ctx.stroke();
    ctx.fillStyle = isActive ? '#4a9eff' : '#ffffff';
    ctx.font = 'bold 9px monospace'; ctx.textAlign = 'center'; ctx.textBaseline = 'middle';
    ctx.fillText(`${i + 1}`, t.pos.x, cy);
  }
...
// ============================================================
// Gear button + Pause overlay — imported from shared BattleUI
// ============================================================

import { drawGearButton, drawPauseOverlay, hitTestGearButton, hitTestPauseResume, hitTestPauseQuit } from './BattleUI';
export { hitTestGearButton, hitTestPauseResume, hitTestPauseQuit };
